...
function ConsultaModal({
  ...
  const handleConsultar = async () => {
    ...
    setResult(null);
    try {
      const data = await module.apiCall(params);
      setResult(data);
    } catch (e: any) {
      ...
      >
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-violet-300 text-sm">{module.description}</p>
          </div>
        ...
        <div className="overflow-y-auto flex-1">
          ...
          <div className="p-6 space-y-4">
            {module.fields.length === 0 ? (
              <p className="text-slate-400 text-sm text-center py-4">Este módulo não requer parâmetros.</p>
            ) : (
          ...
          {/* Error */}
          {error && (
            <div className="mx-6 mb-4 px-4 py-3 rounded-xl flex items-start gap-2" style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)" }}>
          ...
          {result && (
            <div className="mx-6 mb-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-emerald-400 text-sm font-semibold">Resultado encontrado</span>
              </div>
              <pre
                className="rounded-xl p-4 text-xs overflow-auto max-h-64"
                style={{
        ...
        >
          <button
            onClick={handleClear}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm text-red-400 hover:bg-red-500/10 transition-all"
          >
            <X className="w-4 h-4" />
            Limpar
          </button>
