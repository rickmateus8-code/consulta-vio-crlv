...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const SidebarContent = ({ mobile = false }: { mobile?: boolean }) => (
    <div
      className={`flex flex-col h-full ${mobile ? "w-72" : collapsed ? "w-20" : "w-64"} 
        bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-r border-gray-200/80 dark:border-gray-800/80 transition-all duration-250`}
    >
      <div className="flex items-center justify-between px-4 py-3.5 border-b border-gray-200/80 dark:border-gray-800/80 min-h-[64px]">
        {(!collapsed || mobile) && (
        ...
        {!mobile && (
          <button onClick={() => setCollapsed(c => !c)} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
        ...
        <div className="space-y-2">
          <div className={`${collapsed && !mobile ? "flex justify-center" : ""}`}>
            <button
              ...
              className={`flex items-center gap-2 rounded-xl font-bold text-sm btn-glow-blue active:scale-95 ${collapsed && !mobile ? "w-10 h-10 justify-center p-0" : "w-full px-4 py-2.5 justify-center"}`}
            >
              <FilePlus className="w-4.5 h-4.5 flex-shrink-0" />
      ...
      <nav className="flex-1 px-3 py-2 space-y-1 overflow-y-auto custom-scrollbar">
        ...
        <div className="pt-4 mt-4 border-t border-gray-100 dark:border-gray-800">
          <button 
            ...
            className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-black text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-all ${collapsed && !mobile ? "justify-center w-full" : "w-full"}`}
          >
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative bg-[#0b1120]">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3 glass-header shadow-sm z-30">
          <div className="flex items-center gap-3">
            ...
            <img src="/assets/logo-text.webp" alt="DocMaster" className="h-7 w-auto object-contain" style={{ maxHeight: "28px", maxWidth: "140px", objectFit: "contain" }} onClick={() => setLocation("/dashboard")} />
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setShowNovoDocModal(true)} className="flex items-center justify-center btn-glow-blue p-2 rounded-xl transition-colors shadow-sm"><FilePlus className="w-4 h-4" /></button>
            <div onClick={() => handleOpenRecarregaModal(false)} className="flex items-center gap-1.5 badge-pill-emerald px-2.5 py-1.5 cursor-pointer active:scale-95 transition-transform">
              <Wallet className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /><span className="text-[11px] font-black">{balanceFormatted}</span>
            </div>
          </div>
        </header>
      ...
      <a href="https://wa.me/5511965355468?text=Suporte" target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 z-40 w-14 h-14 bg-emerald-500 hover:bg-emerald-600 active:scale-95 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all">
      ...
      {showHistoricoModal && (
        <div className="fixed inset-0 z-[9999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowHistoricoModal(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-lg flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300" onClick={e => e.stopPropagation()}>
